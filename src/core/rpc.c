...
int rpc_send_async_response(rpc_async_context_t *context, uint8_t status, 
    ...
    ROOLE_LOG_DEBUG("[RPC] ReqID %u, FuncID %u, Execution: %.2f us", 
    ...
    if (send_result != 0) {
        ROOLE_LOG_ERROR("[RPC] Failed to send response on FD %d (Request %u)", 
...
static void process_buffered_data(rpc_channel_t *channel) {
    ...
    if (g_service_table == NULL) {
        ROOLE_LOG_ERROR("[RPC] Service table not initialized. Dropping requests");
        channel->rx_data_len = 0;
        return;
    }
    ...
    while (channel->rx_data_len - processed_bytes >= RPC_HEADER_SIZE) {
        ...
        if (rpc_unpack_header(channel->rx_buffer + processed_bytes, &header) != 0) {
            ROOLE_LOG_ERROR("[RPC] Invalid header on FD %d. Dropping buffer", channel->socket_fd);
            channel->rx_data_len = 0;
            return;
        }
        ...
        if (!context) {
            ROOLE_LOG_ERROR("[RPC] Out of memory for async context");
            return; 
        }
        ...
        if (handler != NULL) {
            ...
            if (handler(context, in_payload, payload_len) == 0) {
                ROOLE_LOG_DEBUG("[RPC] Recv (Total: %" PRIu64 ", ID: %u, Func: %u). Async started", 
            ...
            // Function not found
            ROOLE_LOG_WARN("[RPC] Function %u not found", func_id);
            ...
            if (events[i].data.fd == listener_fd) {
                ...
                if (events[i].events & EPOLLIN) {
                    ...
                    if (!close_connection && channel->rx_data_len == channel->rx_buffer_size) {
                        ROOLE_LOG_ERROR("[RPC] Receive buffer full on FD %d", client_fd);
                        close_connection = 1;
                    }
                }
